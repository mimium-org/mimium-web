...
### Support for Multi-Channel Hardware and Audio Input

Previously, mimium only anticipated audio output devices with 1-2 channels, sometimes causing errors when hardware input/output channels were multi-channel. This issue has been fixed. Additionally, by specifying floats or tuples/records of floats as arguments to the `dsp` function, you can now receive hardware audio input and use it like an effect.
Support for multi-channel hardware was implemented thanks to a contribution by [@karnpapon](https://github.com/karnpaon).
...
### MIDI input function: `bind_midi_note_mote` -> `midi_note_mono!`([#163])
