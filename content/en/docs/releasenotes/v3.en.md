...
Default values work with parameter pack syntax. You can use `{}` to accept all default values and specify only the parameters you want to override. Currently, this works for direct function calls (non-closure contexts).
...
Two primitives enable multi-stage computation. **Quote** (`` `expr ``) marks an expression for evaluation in the next stage. **Splice** (`$expr`) evaluates and inserts an expression from the previous stage. The **macro expansion syntax** (`f!(args)`) provides syntactic sugar for `${f(args)}`.
...
This approach provides clear separation between compile-time and runtime code with less nesting than expression-level quotes. It enables better organization for complex macro systems while maintaining type-checked macro expansion.
...
The system provides several key advantages. There is no audio interruption—delay tails and reverb continue naturally. The smart state transfer updates only the changed parts while preserving feedback loops so self-oscillating systems continue oscillating. This enables true live coding where you can modify parameters and structure in real-time.
...
Currently, external I/O state (like GUI sliders) is reset on recompilation. Future versions will support persistent external state.
...
The language server provides semantic highlighting with proper syntax highlighting based on the compiler's tokenizer. It includes error diagnostics for real-time error checking and reporting as you type. VSCode integration works with the mimium VSCode extension v2.3+.

The implementation is built on the `tower-lsp` framework and reuses mimium's existing compiler infrastructure. It provides a foundation for future features like auto-completion and go-to-definition.
...
### Audio File Loading: `gen_sampler_mono` → `Sampler_mono!` (#161)
...
### Probe Function: `make_probe` → `Probe!` (#156)
