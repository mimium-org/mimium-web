...
```mimium{height="500px",label="cascadeosc.mmm"}
#stage(main)
...
fn cascade (n,gen){
    if (n>0.0){
        ...
        `|rate| rate - ($gen)(rate/1.5) * rate * $multiplier  
                    |> $cascade(n - 1.0 ,gen)
    }else{
        `|rate| ($gen)(rate)
    }
...
また、Luaのようにホスト言語上でのネイティブ拡張を簡単に定義できるほか、Rust言語ソースコードへのトランスパイルも可能なため、ゲームエンジンやアプリケーションの中に埋め込んで簡単に利用できます。
